...
{
		int count = 0;																											/* Count the string length. */
    uint8_t TempChar;
		while(str[count] != '\0'){
			count++;
		}
...
/******************************************************************************
* Function Name  : GUI_Build
* Description    : 
* Input          : game_status describe the current status of the game. 
* Output         : None
* Return         : None
* Attention		 : None
*******************************************************************************/
	
int current_score = 0;																																/* Current score value. */
char score[sizeof(current_score)];

int IA_current_score = 0;																															/* IA_Current score value. */
char IA_score[sizeof(IA_current_score)];

extern uint16_t IA_start_paddle;
extern uint16_t IA_paddle_base;

void GUI_Build(uint8_t game_status)
{
	int i = 0;

	uint16_t x_right= 235;																																/* Right border value. */
	uint16_t x_left = 0;																																	/* Left border value. */
	...
	switch(game_status){
		case 0:
			LCD_Clear(Black);
			GUI_Text(0, 120, (uint8_t *) " Press KEY1 to start the game ", White, Black);
			break;
		/* Game start. */
		case 1:
			LCD_Clear(Black);
			for(i = 0; i < 10; i++){
				if(i < 5) {
					LCD_DrawLine(x_right, 0, x_right, 319, Red);																	/* Right border creation. */
					LCD_DrawLine(x_left, 0, x_left, 319, Red);																		/* Left border creation. */
					x_right++;
					x_left++;
				}
			}
		/* IA paddle creation. */	
		for(i = 0; i < 10; i++){																					
			LCD_DrawLine(IA_start_paddle, IA_paddle_base, IA_start_paddle + 40, IA_paddle_base, Green);					
			IA_paddle_base++;
		}
		IA_paddle_base = 32;
		...
		case 2:
			LCD_Clear(Black);
			GUI_Text(10, 30, (uint8_t *) " Score Player 1: ", White, Black);
			GUI_Text(150, 30, (uint8_t *) score, White, Black);
			GUI_Text(10, 50, (uint8_t *) " Score IA : ", White, Black);
			GUI_Text(150, 50, (uint8_t *) IA_score, White, Black);
			GUI_Text(80, 120,(uint8_t *) " Pause ", White, Black);
			GUI_Text(0, 140, (uint8_t *) "Press KEY2 to restart the game", White, Black);
			GUI_Text(0, 160, (uint8_t *) "Press INTO to reset the game ", White, Black);
			break;
		/* You lose. */
		case 3:			
			if(current_score == 5){
				GUI_Text(75, 240, (uint8_t *) " You Win! ", White, Black);
				GUI_TextReverse(85, 80, (uint8_t *) " You Lose! ", White, Black);
			} else if(IA_current_score == 5){
				GUI_TextReverse(85, 80, (uint8_t *) " You Win! ", White, Black);
				GUI_Text(75, 240, (uint8_t *) " You Lose! ", White, Black);
			}
			break;
		default:
			break;
